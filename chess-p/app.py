...
#     cursor.execute("""
#         SELECT gd.game_id, gd.white_player, gd.black_player, gd.white_result, gd.black_result, sg.chess_com_game_id
#         FROM game_data gd
...
#     return render_template('saved_games.html', games=saved_games)
...
#         cursor.execute("DELETE FROM saved_games WHERE chess_com_game_id = %s AND player_id = %s", (game_id, session['user_id']))
#         cursor.execute("DELETE FROM game_data WHERE game_id = %s", (game_id,))
#         connection.commit()
...
# @app.route('/view_games', methods=['POST'])
# def view_games():
#     if 'user_id' not in session:
#         return redirect(url_for('login'))
...
#             if latest_month_game_data.status_code == 200:
#                 games = latest_month_game_data.json()["games"]
#                 return render_template('view_games.html', games=games, username=username)
#             else:
#                 return render_template('index.html', error=f"Couldn't fetch latest month game data. Status code:{latest_month_game_data.status_code}", username=username)
#         else:
#             return render_template('index.html', error=f"Archives are empty for the user {username}", username=username)
...
#     print(f"Received game data: {game_data}")  # Debugging line
...
#         print(f"Parsed game data: {game}")  # Debugging line
